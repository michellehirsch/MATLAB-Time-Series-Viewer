...
% set(dd,'DropHandles',[handles.axes handles.AddTimeVectorsFrame handles.AddTimeVectorsText]);
...
set(dd,'DropHandles',[handles.AddTimeVectorsFrame handles.AddTimeVectorsText]);
...
%             handles.dd = set(handles.dd,'DropHandles',[handles.axes(1) handles.TimeSelectPopup]);
            handles.dd = set(handles.dd,'DropHandles',[handles.axes(1) handles.AddTimeVectorsFrame handles.AddTimeVectorsText]);
            handles.dd = set(handles.dd,'DropValidDrag','all');
            ...
            case 'uicontrol'        % Parameter List
                ...
                for ii=1:length(newparms)
                    ...
                    lh = [lh line(time, data, ...
                        'Parent',drop, ...
                        'Color',colors(cind,:))]; %#ok<AGROW>
                    
                    % Add to list of parameters we actually plotted
                    keepers = keepers+1;
                    newparms_keep{keepers,1} = newparms{ii}; %#ok<AGROW>
...
%     set(ax(1:NoAx-1),'XTick',[]);
    ...
    if iscell(showkids)
        showkids = vertcat(showkids{:});
    end;
    hidekids = get(ax(NoAx+1:end),'Children');
    if iscell(hidekids)
        hidekids = vertcat(hidekids{:});
    end;
...
%     handles.dd = set(handles.dd,'DropHandles',[ax(1:NoAx) handles.TimeSelectPopup]);
    handles.dd = set(handles.dd,'DropHandles',[ax(1:NoAx) handles.AddTimeVectorsFrame handles.AddTimeVectorsText]);
    handles.dd = set(handles.dd,'DropCallbacks',@localDropCallback);
    handles.dd = set(handles.dd,'DropValidDrag','all');
...
%         linkedzoom(handles.axes,'onx',{@localZoomCallback,handles.figure1});
        ...
        %Turn on zoom
        linkaxes(handles.axes,'x');
        z = zoom(handles.figure1);
        set(z,'Enable','on')
        set(z,'ActionPostCallback',@localZoomCallback)
...
%         localTurnOffZoom(handles);
    ...
    % This is called after each zoom update.
